...
class GaussianBlur2D(tf.keras.layers.Layer):
    def __init__(self, kernel_size=3, sigma=1.0, **kwargs):
        super(GaussianBlur2D, self).__init__(**kwargs)
        self.kernel_size = kernel_size
        self.sigma = sigma

    def build(self, input_shape):
        self.kernel = self.get_kernel(input_shape[-1])
        super(GaussianBlur2D, self).build(input_shape)

    def call(self, inputs):
        return tf.nn.depthwise_conv2d(inputs, self.kernel, strides=[1, 1, 1, 1], padding='SAME')

    def get_kernel(self, num_channels):
        x = tf.range(-self.kernel_size // 2 + 1.0, self.kernel_size // 2 + 1.0)
        g = tf.exp(-(x ** 2) / (2 * self.sigma ** 2))
        g_norm = g / tf.reduce_sum(g)
        g_kernel = tf.einsum('i,j->ij', g_norm, g_norm)
